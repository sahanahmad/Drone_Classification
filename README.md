...
## Why It Struggles on Real Images

The detector was trained on real labeled drone images, but real-world performance remains limited. The core limitation is the feature representation itself — not the data.

### The Haar feature limitation

Haar filters detect edges and contrast patterns within fixed rectangular regions. They have no understanding of shape, geometry, or spatial context. This creates three concrete failure modes on real images:

**1. Viewpoint sensitivity**
A real drone photographed at an angle produces a silhouette that looks nothing like the same drone seen from directly below. Haar responses change dramatically with viewpoint, and the classifier has no way to understand that two very different response patterns belong to the same object.    



**2. Occlusion and clutter**
When a drone is partially occluded — by a tree branch, a building edge, or another object — the Haar responses in that window are corrupted. Unlike learned deep features, Haar filters cannot reason about partial visibility or reconstruct missing structure.

**3. Background interference**
Complex real-world backgrounds (tree lines, rooftops, power lines) contain strong edges and contrast boundaries that produce Haar responses nearly identical to those from drone regions. The classifier cannot distinguish between a drone edge and a structurally similar background edge.

These are fundamental limitations of handcrafted rectangular filters — they capture local contrast but not the semantic structure of an object. This is precisely the gap that deep learning (CNNs) addresses by learning hierarchical, shape-aware features directly from data.
